...
//  -1: equals
// >-1: first diff index
int compare(const char *file_name_1, const char *file_name_2) {
  int result = -1;

  char *buff_1 = (char *) malloc(COMPARE_BUFFER_SIZE);
  char *buff_2 = (char *) malloc(COMPARE_BUFFER_SIZE);
  ...
  while (result == -1) {
    ...
    for (int i = 0; i < COMPARE_BUFFER_SIZE / sizeof(int64_t); i++) {
      ...
      for (int j = i * sizeof(int64_t); j < (i + 1) * sizeof(int64_t); j++)
...
int main(const int argc, const char **argv) {
  int result = compare("a.txt", "b.txt");
  printf("\nCompare result: %i\n\n", result);
